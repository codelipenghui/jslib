...
Ext.onReady(function() {
	...
	var store_groupTree = Ext.create("Ext.data.TreeStore", {
        ...
        listeners: {
        	"load": function(thiz, node, records, successful, eOpts){
        		if (node.get("id") == 1) {
        			node.insertChild(0, Ext.create("GroupTreeBean", { id: 0, text: "未分配", leaf: true, groupId: 0, groupName: "未分配", parentId: 1 }));
        		}
        	}
        }
    });
    ...
    /** 用户详情类型 */
    var store_user_detail_type = Ext.create("Ext.data.Store",{
        model: "ValueNameBean",
        data: [
            {value:1, name:"手机号码"},
            {value:2, name:"邮箱地址"},
    ...
    var store_user_detail_selected = Ext.create("Ext.data.Store", {
        model: "UserDetailBean",
        proxy: {
            type: "ajax",
            url: "../user/getDetailByUserId.do",
            extraParams: {"userId": 0}
        },
        autoLoad: true
    })
    /** -------------------------------------视图区------------------------------------- */
    ...
    var grid_userList = Ext.create("Ext.grid.Panel", {
        ...
        listeners: {
        	selectionchange: function(thiz, selected, eOpts ){
        		if( selected.length > 0 && selected[0].data.userId != 1 ){
    ...
    var formPanel_userEdit = Ext.create("Ext.form.Panel", {
        ...
        items: [
           ...
           {xtype: "container", layout: "hbox", margin: "0 0 5 0",  items: [
				{id: "user_treepicker_group", xtype: "treepicker", width:230, labelWidth: 60, fieldLabel: "组织结构", allowBlank: false, emptyText: "请选择组织结构", invalidText: "请选择组织结构！", forceSelection: true, editable: false, displayField: 'groupName', valueField: 'groupId', 
					store: store_groupTree_picker
				},
				{id: "user_combobox_roleId", xtype: "combobox",    width:230, labelWidth: 60, fieldLabel: "权限角色", allowBlank: false, emptyText: "请选择权限角色", invalidText: "请选择权限角色！", queryMode: 'local', typeAhead: true, displayField: 'roleName', valueField: 'roleId', 
    ...
    var grid_userDetailSelectedList = Ext.create("Ext.grid.Panel", {
        store : store_user_detail_selected,
        height : 200,
        width : 550,
        columns : [
            { text: "序号",  width : 50, align : "center", xtype : "rownumberer"},
            { text: "名称", width : 250, align : "center", dataIndex : "type",flex: 1, field: {xtype: 'combobox', store: store_user_detail_type,displayField: "name", valueField: "value", queryMode: "local", forceSelection: true },renderer: edit_user_detail_type_renderer},
            { text: "内容", width: 250, align: "center", dataIndex : "value", flex: 1, field: {xtype: 'textfield', emptyText:"请输入..."}}
        ],
        dockedItems: [{
            xtype: 'toolbar',
            items: [{
                text: '增加',
                icon : jslibPath + "ext4.1/icons/add.png",
                handler: function(){
                    // empty record
                    store_user_detail_selected.insert(0, new UserDetailBean());
                }
            }, '-', {
                itemId: 'delete',
                text: '删除',
                icon : jslibPath + "ext4.1/icons/delete.png",
                disabled: false,
                handler: function(){
                    var selection = grid_userDetailSelectedList.getSelectionModel().getSelection()[0];
                    if (selection) {
                        store_user_detail_selected.remove(selection);
                    }
                }
            }]
        }],
    ...
    var tab_user_edit_all = Ext.create("Ext.tab.Panel", {
        width: 550,
        items: [
            {title : "基本信息" ,border: false, frame: false, items: [ formPanel_userEdit]},
            {title : "详细信息" ,border: false, frame: false, items: [ grid_userDetailSelectedList]},
        ]
    })
    ...
    function userGrid_addHandler(){
        ...
        store_user_detail_selected.removeAll();
        ...
        tab_user_edit_all.setActiveTab(0);
    ...
    function userGrid_editHandler(){
    	if (grid_userList.getSelectionModel().hasSelection()) {
            ...
            store_user_detail_selected.proxy.extraParams = {"userId" : record.data.userId};
            store_user_detail_selected.load();
    ...
    function userForm_submitHandler(){
        var detailIsSet = false;
        var detailTypes = null;
        var detailValues = null;
        for (var i = 0; i < store_user_detail_selected.getCount(); i++) {
            detailType = store_user_detail_selected.getAt(i).data.type;
            detailValue = store_user_detail_selected.getAt(i).data.value;
            if(detailValue == "" || detailValue == null || detailValue == undefined){
                detailValue = " ";
            }
            if(detailType > 0){
                if(detailIsSet == false){
                    detailTypes = detailType;
                    detailValues = detailValue;
                    detailIsSet = true;
                }else {
                    detailTypes = detailTypes + "," + detailType;
                    detailValues = detailValues + "," + detailValue;
                }
            }
        }
    ...
    function resetForm_submitHandler(){
    	if (!Ext.getCmp("reset_text_password_1").isValid()) {
			...
			$.post("../user/userPasswordReset.do", {
                ...
                password: Ext.getCmp("reset_text_password_2").getValue()
			}, function(data){
				if(data.success){
    ...
    /** 已选用户详情类型列渲染 */
    function edit_user_detail_type_renderer(value, cellmeta, record){
        var types = store_user_detail_type.data.items;
        for(var i = 0; i < types.length; i++){
            if(value == types[i].data.value){
                return types[i].data.name;
            }
        }
    }
