...
Ext.onReady(function() {
    ...
    var grid_version = Ext.create("Ext.grid.Panel", {
        ...
        tbar: new Ext.Toolbar({
			...
			items : [
				...
				{ width: 55,  text: "下载", icon: jslibPath + "ext4.1/icons/down.png", handler: version_downLoadHandler}, "-",
    ...
    var formPanel_edit_version = Ext.create("Ext.form.Panel", {
        ...
        items: [
            ...
            {id: "text_edit_uploadFile",   xtype: "filefield", name: "uploadFile",  fieldLabel: "文件上传", emptyText: "请选择zip、rar或apk文件", invalidText: "上传的文件类型必须是zip、rar或apk！", buttonText: "浏览...", regex: new RegExp("\.(msi|zip|rar|apk|ipa|MSI|ZIP|RAR|APK|IPA)$")},
	...
	function version_downLoadHandler(){
		if(grid_version.getSelectionModel().hasSelection()){
			var record = grid_version.getSelectionModel().getSelection()[0];
			window.open(record.data.downUrl);
		}else{
			liming.message_info("请先选择数据再操作！");
		}
